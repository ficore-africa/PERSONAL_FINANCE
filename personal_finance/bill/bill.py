            ...
            if action == 'add_bill':
                ...
                if action == 'update_bill':
                    ...
                    try:
                        ...
                        # Create transaction record when bill is marked as paid
                        if new_status == 'paid':
                            try:
                                from helpers.transaction_helpers import create_bill_payment_transaction
                                bill_data = bill.copy()
                                bill_data['_id'] = bill_id
                                transaction_id = create_bill_payment_transaction(
                                    db, 
                                    bill_data, 
                                    session_id=session.get('sid', 'unknown')
                                )
                                if transaction_id:
                                    current_app.logger.info(f"Created transaction {transaction_id} for bill payment {bill_id}", 
                                                           extra={'session_id': session.get('sid', 'unknown')})
                            except Exception as e:
                                current_app.logger.error(f"Error creating bill payment transaction: {str(e)}", 
                                                        extra={'session_id': session.get('sid', 'unknown')})
