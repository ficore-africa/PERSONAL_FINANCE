...
from helpers.transaction_helpers import create_shopping_expense_transaction
import json
    ...
    try:
        with db.client.start_session() as mongo_session:
            with mongo_session.start_transaction():
                ...
                if result.modified_count > 0:
                    # Create transaction record when item is marked as bought
                    if new_status == 'bought':
                        shopping_list = db.shopping_lists.find_one({'_id': ObjectId(item['list_id'])})
                        if shopping_list:
                            try:
                                create_shopping_expense_transaction(db, item, shopping_list, session_id)
                            except Exception as e:
                                logger.warning(f"Failed to create transaction for shopping item {item_id}: {str(e)}")
