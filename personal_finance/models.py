            ...
            collection_schemas = {
                    ...
                    'validator': {
                        '$jsonSchema': {
                            ...
                            'properties': {
                                'user_id': {'bsonType': 'string'},
                                'session_id': {'bsonType': ['string', 'null']},
                                'budget_name': {'bsonType': ['string', 'null'], 'maxLength': 100},
                                'income_items': {
                                    'bsonType': 'array',
                                    'items': {
                                        'bsonType': 'object',
                                ...
                                'total_income': {'bsonType': ['double', 'int'], 'minimum': 0},
                                'total_expenses': {'bsonType': ['double', 'int'], 'minimum': 0},
                                'total_investments': {'bsonType': ['double', 'int'], 'minimum': 0},
                                'total_savings': {'bsonType': ['double', 'int'], 'minimum': 0},
                                'total_dependents': {'bsonType': ['double', 'int'], 'minimum': 0},
                                'surplus_deficit': {'bsonType': ['double', 'int']},
                                'created_at': {'bsonType': 'date'},
                                'updated_at': {'bsonType': ['date', 'null']},
                                # Legacy fields for backward compatibility
                                'income': {'bsonType': ['double', 'int', 'null'], 'minimum': 0},
                                'fixed_expenses': {'bsonType': ['double', 'int', 'null'], 'minimum': 0},
                                'variable_expenses': {'bsonType': ['double', 'int', 'null'], 'minimum': 0},
                ...
                'transactions': {
                    'validator': {
                        '$jsonSchema': {
                            'bsonType': 'object',
                            'required': ['user_id', 'type', 'category', 'amount', 'description', 'timestamp', 'status'],
                            'properties': {
                                'user_id': {'bsonType': 'string'},
                                'session_id': {'bsonType': ['string', 'null']},
                                'type': {'enum': ['income', 'expense', 'bill_payment', 'shopping', 'budget_allocation', 'investment', 'savings', 'transfer']},
                                ...
                                'reference_type': {'enum': ['bill', 'budget', 'shopping_list', 'shopping_item', 'manual', 'null'], 'bsonType': ['string', 'null']},
                                'metadata': {
                                    'bsonType': ['object', 'null'],
                                    'properties': {
                                        'bill_name': {'bsonType': ['string', 'null']},
                                        'shopping_list_name': {'bsonType': ['string', 'null']},
                                        'budget_name': {'bsonType': ['string', 'null']},
                    ...
                    'indexes': [
                        {'key': [('user_id', ASCENDING), ('timestamp', DESCENDING)]},
                        {'key': [('type', ASCENDING), ('timestamp', DESCENDING)]},
                        {'key': [('category', ASCENDING), ('timestamp', DESCENDING)]},
                        {'key': [('status', ASCENDING)]},
                        {'key': [('reference_type', ASCENDING), ('reference_id', ASCENDING)]},
                        {'key': [('created_at', DESCENDING)]}
                    ]
                }
            }
...
def get_transactions(db, filter_kwargs, limit=None, sort_field='timestamp', sort_direction=DESCENDING):
    """
    Retrieve transaction records based on filter criteria.
    ...
    """
    try:
        query = db.transactions.find(filter_kwargs).sort(sort_field, sort_direction)
        if limit:
            query = query.limit(limit)
        return list(query)
    except Exception as e:
        logger.error(f"{trans('general_transactions_fetch_error', default='Error getting transactions')}: {str(e)}", 
                     exc_info=True, extra={'session_id': 'no-session-id'})
        raise

def get_recent_transactions(db, user_id, limit=10):
    """
    Get recent transactions for a user.
    
    Args:
        db: MongoDB database instance
        user_id: User ID (None for admin to get all transactions)
        limit: Number of recent transactions to return
    
    Returns:
        list: List of recent transaction records
    """
    try:
        query = {'user_id': user_id} if user_id else {}
        return get_transactions(db, query, limit=limit)
    except Exception as e:
        logger.error(f"Error getting recent transactions for user {user_id}: {str(e)}", 
                     exc_info=True, extra={'session_id': 'no-session-id'})
        return []

def get_transaction_summary(db, user_id, start_date=None, end_date=None):
    """
    ...
    Returns:
        dict: Transaction summary with totals by type and category
    """
    try:
        ...
        for result in results:
            ...
            if transaction_type == 'income':
                ...
                summary['total_expenses'] += result['total_amount']
            ...
            for category in result['categories']:
                ...
                summary['by_category'][cat_name]['total_amount'] += category['total_amount']
                summary['by_category'][cat_name]['total_count'] += category['count']
                summary['by_category'][cat_name]['types'][transaction_type] = category
