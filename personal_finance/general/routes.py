...
@login_required
def home():
    """Personal homepage with recent activity data."""
    ...
    try:
        # Get recent activity data for the homepage
        ...
        for transaction in recent_transactions:
            activity_icon = 'bi-arrow-up-circle' if transaction['type'] == 'income' else 'bi-arrow-down-circle'
            activity_type = transaction['type'].replace('_', ' ').title()
            
            formatted_activities.append({
                'description': f"{activity_type}: {transaction['description']}",
        ...
        current_app.logger.error(f"Error loading home page data: {str(e)}")
        return render_template(
            'general/home.html',
            title=trans('general_business_home', lang=session.get('lang', 'en')),
            recent_activities=[]
