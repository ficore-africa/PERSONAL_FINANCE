...
def get_recent_activities(user_id=None, is_admin_user=False, db=None, session_id=None, limit=2):
    ...
    try:
        ...
        # Fetch recent transactions (most important for financial insights)
        transactions = db.transactions.find(query).sort('timestamp', -1).limit(10)
        for transaction in transactions:
            if not transaction.get('timestamp') or not transaction.get('description'):
                logger.warning(f"Skipping invalid transaction record: {transaction.get('_id')}", extra={'session_id': session_id or 'unknown', 'ip': request.remote_addr or 'unknown'})
                continue
            
            transaction_type = transaction.get('type', 'transaction')
            icon_map = {
            ...
            activities.append({
                'type': 'transaction',
                'description': transaction.get('description', 'Financial transaction'),
                ...
                'icon': icon_map.get(transaction_type, 'bi-cash-coin')
            })
